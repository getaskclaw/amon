...
## 第 5 轮盲检（对 0.1.2 增量）— 两席再次独立 **SHIP**

范围只有 `git diff cc80fec..1ca6e89`；工单要求**推翻**作者列的 5 条断言。

| 席 | 结论 | 它自己做的 |
|---|---|---|
| audit | **SHIP** | 5 条断言全部没被推翻；自建 4 类夹具（遗留 map 2 组 → `legacy or corrupt file, groups: 2`；截断非 JSON → `unreadable or corrupt`；有效 File + 错签名 3 行 → `different surface, groups: 3`；**把 `conn-state.json` 做成目录** → `unreadable or corrupt`，并实测 Windows 下读目录不是 `NotFound`）；确认缺文件零行、`--conn-sec 0` 零行零 sidecar 零事件、baseline 拒绝消息逐字未变、同表面续接零重报 |
| forge（第二席） | **SHIP** | 自建 15 个分类边界夹具 + 零轮/续接/节奏/伪造/只读探针；目录名、sig 为数字、rows 为数组、顶层标量、空文件、BOM 全部正确落 `unreadable or corrupt`；混合行逐行过滤只保留有效行；`--conn-sec 0` 三连跑 sidecar 哈希不变；关后开正常续接 |

两席各自列出的"未验证"一致且诚实：真实 `rc!=0` 采样失败、IPv6-only 主机、运行中地址族翻转
（本机无法注入）；节流窗口之外的长时间行为未压测。

### 本轮 4 条非阻塞观察（**均为 delta 之前既有**，处置见下）

1. **sidecar 写入窗口真实**（11s 才落第二笔；硬杀丢更新 → 下次启动重报一次）—— 这就是限制 7 与契约 16 已记录的 B1b 残留窗口。
2. **baseline 陈旧签名不自愈**，每次启动多一行拒绝 —— **有意保留**（见 README 16）：它在持续提示"基线对本表面不可用"，
   自愈等于把这条信号静默掉。换过滤后重跑 `--baseline` 即可消除。
3. **baseline 与 sidecar 同级可信**，手改 baseline 可注入一对 open+close —— 与限制 10 同源，已写进 README 17。
4. **孤儿 `.tmp`**（rename 覆盖目录失败时留下，内容有效、自限、无崩溃）—— 纯观感，记入 README 17。

结论：0.1.2 的增量在**每一条可执行路径**上都与断言一致，没有伪造关闭、没有凭空的拒绝行、关闭就是关闭。
